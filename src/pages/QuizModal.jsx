...
export default function QuizModal({ topic, onClose }) {
  ...
  async function handleSkip() {
    await api.post(`/api/paths/topics/${topic.id}/quiz/skip`)
    onClose()
  }
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            background: selected ? 'rgba(124,58,237,.12)' : 'var(--surface2)',
                            border: `1.5px solid ${selected ? 'var(--accent)' : 'var(--border)'}`,
                            color: selected ? 'var(--accent3)' : 'var(--text)',
                        ...
                        >
                          ...
                          <span style={{ fontSize: 13 }}>{optionText(q, opt)}</span>
                        </button>
                      )
                    })}
                  </div>
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, flexShrink: 0 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: answered > 0 ? 'linear-gradient(135deg,var(--accent),var(--accent2))' : 'var(--surface3)',
                  color: '#fff', fontSize: 13, fontWeight: 700, cursor: answered > 0 ? 'pointer' : 'not-allowed',
                  opacity: answered > 0 ? 1 : 0.4, fontFamily: 'DM Sans,sans-serif',
                  boxShadow: answered > 0 ? '0 4px 16px var(--glow)' : 'none',
