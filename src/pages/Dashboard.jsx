    ...
    <div style={{marginTop:16}}>
      ...
      {loading?<Spin s={14}/>:subs.map(s=>(
        <div key={s.id} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 0',borderBottom:'1px solid var(--border)'}}>
          <button onClick={()=>toggle(s.id)} style={{width:20,height:20,borderRadius:6,flexShrink:0,background:s.completed?'var(--accent)':'transparent',border:`2px solid ${s.completed?'var(--accent)':'var(--border2)'}`,color:'#fff',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>{s.completed?'✓':''}</button>
...
function TaskModal({task,onClose,onSave,me}){
  ...
  const [f,setF]=useState({title:task?.title||'',description:task?.description||'',priority:task?.priority||'MEDIUM',status:task?.status||'TODO',dueDate:task?.dueDate?task.dueDate.split('T')[0]:'',dueTime:task?.dueDate?task.dueDate.split('T')[1]?.slice(0,5)||'09:00':'09:00',recurring:task?.recurring||false,recurringInterval:task?.recurringInterval||'DAILY'})
  ...
  async function parseNlp(){if(!nlpInput.trim())return;setNlpLoading(true);try{const res=await import('../services/api').then(m=>m.default.post('/api/ai/parse-task',{input:nlpInput}));const d=res.data;setF(p=>({...p,title:d.title||p.title,description:d.description||p.description,priority:d.priority||p.priority,dueDate:d.dueDate?d.dueDate.split('T')[0]:p.dueDate,dueTime:d.dueDate?d.dueDate.split('T')[1]?.slice(0,5)||'09:00':p.dueTime}));setNlpInput('');setNlpActive(false)}catch(e){console.error(e)}finally{setNlpLoading(false)}}
  useEffect(()=>{const h=e=>{if(e.key==='Escape')onClose()};window.addEventListener('keydown',h);return()=>window.removeEventListener('keydown',h)},[onClose])
  async function submit(){if(!f.title.trim())return;setSaving(true);try{const dueDate=f.dueDate?`${f.dueDate}T${f.dueTime||'00:00'}:00`:null;await onSave({title:f.title,description:f.description,priority:f.priority,status:f.status,dueDate,recurring:f.recurring,recurringInterval:f.recurring?f.recurringInterval:null})}finally{setSaving(false)}}
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.85)',backdropFilter:'blur(10px)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:16}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box" style={{width:'100%',maxWidth:560,maxHeight:'90vh',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:22,overflow:'hidden',boxShadow:'0 30px 80px rgba(0,0,0,.8)',animation:'modalIn .25s ease',display:'flex',flexDirection:'column'}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 22px',borderBottom:'1px solid var(--border)',background:'linear-gradient(135deg,rgba(124,58,237,.07),transparent)',flexShrink:0}}>
          <div><h2 style={{fontSize:17,fontWeight:700,color:'var(--text)',fontFamily:'Syne,sans-serif'}}>{isEdit?'✎ Edit Task':'⚡ New Task'}</h2><p style={{fontSize:11,color:'var(--muted)',marginTop:2}}>{isEdit?'Update details':'Add to your board'}</p></div>
        ...
        <div style={{display:'flex',gap:4,padding:'10px 22px 0',borderBottom:'1px solid var(--border)',flexShrink:0}}>
          {TABS.map(([key,ic,lb])=>(<button key={key} onClick={()=>setTab(key)} style={{padding:'7px 14px',borderRadius:'9px 9px 0 0',fontSize:12,fontWeight:600,cursor:'pointer',background:tab===key?'var(--surface2)':'transparent',color:tab===key?'var(--accent3)':'var(--muted)',border:tab===key?'1px solid var(--border)':'1px solid transparent',borderBottom:tab===key?'1px solid var(--surface2)':'none',transition:'all .15s'}}>{ic} {lb}</button>))}
        </div>
        <div style={{flex:1,overflowY:'auto',padding:22}}>
          {tab==='details'&&(<>
            ...
            <div style={{padding:14,background:'var(--surface2)',borderRadius:12,border:'1px solid var(--border)'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><div style={{display:'flex',alignItems:'center',gap:9}}><span style={{fontSize:17}}>🔁</span><div><p style={{fontSize:13,fontWeight:600,color:'var(--text)',margin:0}}>Recurring Task</p><p style={{fontSize:11,color:'var(--muted)',margin:0}}>Auto-recreate when completed</p></div></div><button onClick={()=>set('recurring',!f.recurring)} style={{width:42,height:24,borderRadius:12,border:'none',cursor:'pointer',position:'relative',background:f.recurring?'var(--accent)':'var(--surface3)',transition:'background .2s'}}><span style={{position:'absolute',top:2,width:20,height:20,borderRadius:'50%',background:'#fff',transition:'left .2s',left:f.recurring?20:2}}/></button></div>{f.recurring&&(<select style={{...INP,marginTop:12}} value={f.recurringInterval} onChange={e=>set('recurringInterval',e.target.value)}><option value="DAILY">🔁 Daily</option><option value="WEEKLY">📅 Weekly</option><option value="MONTHLY">🗓 Monthly</option></select>)}</div>
          </>)}
          {tab==='subtasks'&&isEdit&&<SubTaskList taskId={task.id}/>}
          {tab==='comments'&&isEdit&&<Comments taskId={task.id} me={me}/>}
        </div>
        {tab==='details'&&(<div style={{display:'flex',gap:10,padding:'16px 22px',borderTop:'1px solid var(--border)',flexShrink:0}}><button onClick={onClose} style={{flex:1,padding:12,borderRadius:10,border:'1px solid var(--border)',background:'var(--surface2)',color:'var(--muted)',fontSize:13,fontWeight:600,cursor:'pointer'}}>Cancel</button><button onClick={submit} disabled={saving||!f.title.trim()} style={{flex:2,padding:12,borderRadius:10,border:'none',background:saving?'var(--surface3)':'linear-gradient(135deg,var(--accent),var(--accent2))',color:'#fff',fontSize:13,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:8,opacity:f.title.trim()?1:.5,cursor:'pointer',boxShadow:saving?'none':'0 4px 16px var(--glow)',transition:'all .2s'}}>{saving?<><Spin s={14}/> Saving…</>:isEdit?'✓ Save Changes':'⚡ Create Task'}</button></div>)}
      </div>
    </div>
  )
}
...
function Kanban({tasks,onEdit,onStatusChange}){
  ...
  const cols=[{key:'TODO',label:'To Do',emoji:'📋',color:'#a78bfa'},{key:'IN_PROGRESS',label:'In Progress',emoji:'⚡',color:'#60a5fa'},{key:'DONE',label:'Done',emoji:'✅',color:'#6bcb77'}]
  return(
    <div className="kgrid" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:14}}>
      {cols.map(col=>{
        const ct=tasks.filter(t=>t.status===col.key)
        return(
          <div key={col.key} className="kcol" style={{background:'var(--surface2)',borderRadius:16,padding:14,border:'1px solid var(--border)',minHeight:300,transition:'all .2s'}} onDragOver={e=>{e.preventDefault();e.currentTarget.classList.add('dragover')}} onDragLeave={e=>e.currentTarget.classList.remove('dragover')} onDrop={e=>{e.preventDefault();e.currentTarget.classList.remove('dragover');if(dragId)onStatusChange(dragId,col.key);setDragId(null)}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14}}><span style={{fontSize:15}}>{col.emoji}</span><span style={{fontSize:13,fontWeight:700,color:col.color}}>{col.label}</span><span style={{marginLeft:'auto',fontSize:10,padding:'2px 7px',borderRadius:6,background:'var(--surface3)',color:'var(--muted)'}}>{ct.length}</span></div>
            {ct.map(task=>{const cfg=PRI[task.priority]||PRI.MEDIUM;const st=(task.subTasks||[]).filter(s=>s.completed).length;const stTotal=(task.subTasks||[]).length;return(<div key={task.id} className="kcard" draggable onDragStart={()=>setDragId(task.id)} style={{background:'var(--surface)',border:'1px solid var(--border)',borderLeft:`3px solid ${cfg.color}`,borderRadius:12,padding:12,marginBottom:10,animation:'fadeUp .25s ease'}} onMouseEnter={e=>e.currentTarget.style.boxShadow='0 4px 16px rgba(0,0,0,.3)'} onMouseLeave={e=>e.currentTarget.style.boxShadow='none'}><p style={{fontSize:13,fontWeight:600,color:'var(--text)',margin:'0 0 8px',lineHeight:1.4}}>{task.title}</p>{stTotal>0&&(<div style={{marginBottom:8}}><div style={{height:3,background:'var(--surface3)',borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',width:`${(st/stTotal)*100}%`,background:'linear-gradient(90deg,var(--accent),var(--accent2))',transition:'width .3s'}}/></div><p style={{fontSize:10,color:'var(--muted)',marginTop:2}}>{st}/{stTotal} subtasks</p></div>)}<div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}><span style={{fontSize:10,fontWeight:700,padding:'2px 7px',borderRadius:6,background:cfg.bg,color:cfg.color}}>{cfg.label}</span><div style={{display:'flex',alignItems:'center',gap:6}}>{task.dueDate&&<span style={{fontSize:10,color:'var(--muted)'}}>{new Date(task.dueDate).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</span>}<button onClick={()=>onEdit(task)} style={{width:22,height:22,borderRadius:6,background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--muted)',fontSize:11,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>✎</button></div></div></div>)})}
...
function Calendar({tasks,onEdit}){
  ...
  function getDay(d){return tasks.filter(t=>{if(!t.dueDate)return false;const td=new Date(t.dueDate);return td.getFullYear()===y&&td.getMonth()===m&&td.getDate()===d})}
  const selTasks=sel?getDay(sel):[]
  return(
    <div style={{animation:'fadeUp .3s ease'}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}><button onClick={()=>setCur(new Date(y,m-1,1))} style={{width:34,height:34,borderRadius:9,background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--muted)',fontSize:18,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>‹</button><h3 style={{fontSize:18,fontWeight:800,color:'var(--text)',fontFamily:'Syne,sans-serif'}}>{MONTHS[m]} {y}</h3><button onClick={()=>setCur(new Date(y,m+1,1))} style={{width:34,height:34,borderRadius:9,background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--muted)',fontSize:18,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>›</button></div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:4}}>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(d=><div key={d} style={{textAlign:'center',fontSize:10,fontWeight:700,color:'var(--muted)',padding:'6px 0',letterSpacing:'1px'}}>{d}</div>)}</div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
        {Array.from({length:first}).map((_,i)=><div key={`e${i}`}/>)}
        {Array.from({length:days}).map((_,i)=>{const d=i+1;const dt=getDay(d);const isToday=today.getFullYear()===y&&today.getMonth()===m&&today.getDate()===d;const isSel=sel===d;return(<div key={d} className="cal-day" onClick={()=>setSel(sel===d?null:d)} style={{minHeight:72,padding:7,borderRadius:12,cursor:dt.length?'pointer':'default',transition:'all .15s',background:isSel?'rgba(124,58,237,.1)':isToday?'rgba(124,58,237,.05)':'var(--surface)',border:`1px solid ${isSel?'var(--accent)':isToday?'rgba(124,58,237,.3)':'var(--border)'}`}}><p style={{fontSize:12,fontWeight:isToday?800:500,color:isToday?'var(--accent2)':'var(--muted)',marginBottom:3}}>{d}</p>{dt.slice(0,2).map(t=>{const cfg=PRI[t.priority]||PRI.MEDIUM;return(<div key={t.id} style={{fontSize:9,padding:'2px 4px',borderRadius:4,marginBottom:2,background:cfg.bg,color:cfg.color,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{t.title}</div>)})}{dt.length>2&&<p style={{fontSize:9,color:'var(--muted)'}}>+{dt.length-2}</p>}</div>)})}
      </div>
      {sel&&selTasks.length>0&&(<div style={{marginTop:16,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:16,padding:16,animation:'fadeUp .25s ease'}}><p style={{fontSize:13,fontWeight:700,color:'var(--text)',marginBottom:12}}>{MONTHS[m]} {sel} — {selTasks.length} task{selTasks.length!==1?'s':''}</p>{selTasks.map(t=>{const cfg=PRI[t.priority]||PRI.MEDIUM;const scfg=STATUS[t.status]||STATUS.TODO;return(<div key={t.id} onClick={()=>onEdit(t)} style={{display:'flex',alignItems:'center',gap:10,padding:'10px 12px',borderRadius:10,background:'var(--surface2)',marginBottom:8,cursor:'pointer',borderLeft:`3px solid ${cfg.color}`}}><span style={{flex:1,fontSize:13,color:'var(--text)',fontWeight:500}}>{t.title}</span><span style={{fontSize:10,padding:'2px 8px',borderRadius:6,background:`${scfg.color}18`,color:scfg.color,fontWeight:600}}>{scfg.label}</span><span style={{fontSize:10,padding:'2px 8px',borderRadius:6,background:cfg.bg,color:cfg.color,fontWeight:700}}>{cfg.label}</span></div>)})}</div>)}
      {sel&&selTasks.length===0&&<p style={{marginTop:12,textAlign:'center',fontSize:13,color:'var(--muted)'}}>No tasks due on {MONTHS[m]} {sel}</p>}
    </div>
  )
}

// ─── CHANGED: added onBreakdown prop + title click + 🤖 button ────────────
function TaskRow({task,onToggle,onEdit,onDelete,onBreakdown,idx}){
  ...
  function dueInfo(){if(!task.dueDate)return null;const d=new Date(task.dueDate);const diff=Math.ceil((d-Date.now())/86400000);if(diff<0)return{text:`${-diff}d overdue`,color:'var(--danger)'};if(diff===0)return{text:'Due today',color:'var(--warn)'};if(diff<=2)return{text:`In ${diff}d`,color:'var(--warn)'};return{text:d.toLocaleDateString('en-IN',{day:'numeric',month:'short'}),color:'var(--muted)'}}
  const due=dueInfo()
  function handleDel(){if(confirmDel){onDelete();setConfirm(false)}else{setConfirm(true);setTimeout(()=>setConfirm(false),3000)}}
  return(
    <div className="task-row" onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      ...
      <button onClick={onToggle} style={{width:22,height:22,borderRadius:7,flexShrink:0,background:isDone?cfg.color:'transparent',border:`2px solid ${isDone?cfg.color:'var(--border2)'}`,color:'#fff',fontSize:11,fontWeight:800,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .15s'}}>{isDone?'✓':''}</button>
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
          {/* title click → opens drawer */}
          <p onClick={()=>!isDone&&onEdit&&onEdit()} style={{fontSize:14,fontWeight:500,color:isDone?'var(--muted)':'var(--text)',textDecoration:isDone?'line-through':'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',margin:0,cursor:isDone?'default':'pointer'}}>{task.title}</p>
        ...
        {stTotal>0&&(<div style={{display:'flex',alignItems:'center',gap:6}}><div style={{width:50,height:3,background:'var(--surface3)',borderRadius:3,overflow:'hidden'}}><div style={{height:'100%',width:`${(st/stTotal)*100}%`,background:'var(--accent)',borderRadius:3}}/></div><span style={{fontSize:10,color:'var(--muted)'}}>{st}/{stTotal}</span></div>)}
      ...
      <div style={{display:'flex',gap:5,opacity:hover?1:0,transition:'opacity .15s',flexShrink:0}}>
        {/* 🤖 NEW button */}
        {!isDone&&(
          <button onClick={e=>{e.stopPropagation();onBreakdown&&onBreakdown()}} title="AI: Break into steps"
            style={{height:28,padding:'0 9px',borderRadius:8,background:'rgba(124,58,237,.1)',border:'1px solid rgba(124,58,237,.25)',color:'#a855f7',fontSize:11,fontWeight:600,display:'flex',alignItems:'center',gap:3,cursor:'pointer',transition:'background .15s',whiteSpace:'nowrap'}}
            onMouseEnter={e=>{e.currentTarget.style.background='rgba(124,58,237,.22)'}}
            onMouseLeave={e=>{e.currentTarget.style.background='rgba(124,58,237,.1)'}}>🤖</button>
        )}
        ...
        <button onClick={handleDel} title={confirmDel?'Confirm?':'Delete'} style={{width:28,height:28,borderRadius:8,background:confirmDel?'rgba(255,107,107,.15)':'var(--surface2)',border:`1px solid ${confirmDel?'var(--danger)':'var(--border2)'}`,color:confirmDel?'var(--danger)':'var(--muted)',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .15s'}}>{confirmDel?'!':'✕'}</button>
      </div>
    </div>
  )
}
...
export default function Dashboard(){
  ...
  const NAV_GROUPS={focus:['focus','pomodoro','habits','calendar'],insights:['analytics','weekly','advanced-analytics','matrix'],collaborate:['teams','leaderboard','ai'],tools:['reminders','export','gcal','email-task']}
  ...
  useEffect(()=>{api.get('/api/users/me').then(r=>{setRoleFromApi(r.data.role||'USER');if(user)login(localStorage.getItem('token'),{...user,role:r.data.role,isPro:r.data.isPro})}).catch(()=>{})},[])
  useEffect(()=>{api.get('/api/users/onboarding-status').then(r=>{if(!r.data.done)setShowOnboarding(true)}).catch(()=>{})},[])
  ...
  const offlineState=useOffline(token,import.meta.env.VITE_API_URL,fetchAll)
  ...
  async function handleToggle(id){const t=tasks.find(t=>t.id===id);if(!t)return;const s=t.status==='DONE'?'TODO':'DONE';await api.put(`/api/tasks/${id}`,{title:t.title,description:t.description,priority:t.priority,dueDate:t.dueDate,status:s});fetchAll();flash('success',s==='DONE'?'🎉 Completed!':'↩ Reopened')}
  async function handleDelete(id){await api.delete(`/api/tasks/${id}`);fetchAll();flash('success','Deleted')}
  async function handleStatusChange(id,status){await api.patch(`/api/tasks/${id}/status`,{status});fetchAll();flash('success',`→ ${STATUS[status]?.label}`)}
  function openEdit(task){setEditTask(task);setModal(true)}
  function openNew(){setEditTask(null);setModal(true)}

  const filtered=tasks.filter(t=>{const mf=filter==='ALL'||t.status===filter;const mq=!search||t.title?.toLowerCase().includes(search.toLowerCase())||t.description?.toLowerCase().includes(search.toLowerCase());return mf&&mq})
  ...
  const navItems=[{key:'ALL',icon:'◈',label:'All Tasks',count:tasks.length},{key:'TODO',icon:'○',label:'To Do',count:tasks.filter(t=>t.status==='TODO').length},{key:'IN_PROGRESS',icon:'◉',label:'In Progress',count:tasks.filter(t=>t.status==='IN_PROGRESS').length},{key:'DONE',icon:'✓',label:'Completed',count:done}]
  const fBtn=(key,label,count)=>(<button onClick={()=>setFilter(key)} style={{display:'flex',alignItems:'center',gap:6,padding:'7px 14px',borderRadius:9,fontSize:12,fontWeight:500,cursor:'pointer',background:filter===key?'var(--accent)':'var(--surface2)',color:filter===key?'#fff':'var(--muted)',border:filter===key?'none':'1px solid var(--border)',transition:'all .15s'}}>{label}<span style={{fontSize:10,padding:'1px 6px',borderRadius:5,background:filter===key?'rgba(255,255,255,.2)':'var(--surface3)',color:filter===key?'#fff':'var(--muted)'}}>{count}</span></button>)
      ...
      <div style={{display:'flex',height:'100vh',overflow:'hidden',background:theme==='light'?'#f4f4f9':'var(--bg)',color:theme==='light'?'#1a1a2e':'var(--text)','--bg':theme==='light'?'#f4f4f9':'#0a0a0f','--surface':theme==='light'?'#ffffff':'#111118','--surface2':theme==='light'?'#ededf4':'#1a1a24','--border':theme==='light'?'rgba(0,0,0,.09)':'rgba(255,255,255,.06)','--border2':theme==='light'?'rgba(0,0,0,.13)':'rgba(255,255,255,.1)','--text':theme==='light'?'#1a1a2e':'#f0f0f8','--muted':theme==='light'?'#555577':'#6b6b8a'}}>
        ...
        {sideOpen&&(
          <aside className="sidebar open" style={{width:236,flexShrink:0,background:'var(--surface)',borderRight:'1px solid var(--border)',display:'flex',flexDirection:'column',height:'100%',transition:'transform .25s ease'}}>
            ...
            <nav style={{flex:1,padding:'12px 10px',overflowY:'auto'}}>
              ...
              {[{key:'focus',label:'Focus & Habits',icon:'🎯',items:[['focus','🎯','Daily Focus',null],['pomodoro','⏱','Pomodoro',null],['habits','🔥','Habits',null],['calendar','📅','Calendar',null]]},{key:'insights',label:'Insights',icon:'📊',items:[['analytics','📊','Analytics',null],['advanced-analytics','📈','Deep Analytics',null],['weekly','📋','Weekly Review',null],['matrix','⚡','Priority Matrix',null]]},{key:'collaborate',label:'Teams & Social',icon:'👥',items:[['teams','👥','Teams',null],['leaderboard','🏆','Leaderboard',null],['ai','🤖','AI Assistant',null]]},{key:'tools',label:'Tools',icon:'🔧',items:[['reminders','⏰','Reminders',null],['gcal','📅','Google Cal',null],['email-task','📧','Email→Task',null],['export','📤','Export',null]]}].map(group=>{const isGroupActive=group.items.some(([v])=>v===view);const isOpen=navOpen[group.key];return(<div key={group.key} style={{marginBottom:4}}><button onClick={()=>toggleNav(group.key)} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderRadius:10,cursor:'pointer',background:isGroupActive?'rgba(124,58,237,.06)':'transparent',border:`1px solid ${isGroupActive?'rgba(124,58,237,.12)':'transparent'}`,transition:'all .15s'}}><span style={{fontSize:14}}>{group.icon}</span><span style={{flex:1,fontSize:11,fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:isGroupActive?'var(--accent2)':'var(--muted)',textAlign:'left'}}>{group.label}</span><span style={{fontSize:10,color:'var(--muted)',transition:'transform .2s',transform:isOpen?'rotate(90deg)':'rotate(0deg)'}}>›</span></button>{isOpen&&(<div style={{paddingLeft:8,marginTop:2}}>{group.items.map(([v,ic,lb,badge])=>(<button key={v} className="nav-item" onClick={()=>{setView(v);openGroupForView(v)}} style={{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'8px 10px',borderRadius:9,marginBottom:1,cursor:'pointer',fontSize:12,fontWeight:500,textAlign:'left',transition:'all .12s',background:view===v?'rgba(124,58,237,.12)':'transparent',color:view===v?'var(--accent2)':'var(--muted)',border:view===v?'1px solid rgba(124,58,237,.2)':'1px solid transparent'}}><span style={{width:16,textAlign:'center',fontSize:13}}>{ic}</span><span style={{flex:1}}>{lb}</span>{badge&&<span style={{fontSize:9,padding:'2px 5px',borderRadius:4,background:'linear-gradient(135deg,#7c3aed,#a855f7)',color:'#fff',fontWeight:700}}>{badge}</span>}</button>))}</div>)}</div>)})}
              <button onClick={()=>navigate('/pricing')} style={{width:'100%',display:'flex',alignItems:'center',gap:9,padding:'9px 10px',borderRadius:10,marginBottom:2,cursor:'pointer',fontSize:13,fontWeight:500,textAlign:'left',background:'rgba(255,217,61,.06)',border:'1px solid rgba(255,217,61,.15)',color:'var(--warn)'}}><span style={{width:17,textAlign:'center'}}>⭐</span><span>Upgrade to Pro</span></button>
              ...
              {badges.length>0&&(<div style={{margin:'10px 2px 0',padding:'11px',background:'var(--surface2)',border:'1px solid var(--border)',borderRadius:12}}><p style={{fontSize:9,fontWeight:700,letterSpacing:'2px',textTransform:'uppercase',color:'var(--muted)',marginBottom:8}}>BADGES</p><div style={{display:'flex',flexWrap:'wrap',gap:5}}>{badges.map(b=>(<div key={b.id} title={b.description} style={{padding:'3px 9px',borderRadius:20,fontSize:11,fontWeight:600,background:'rgba(124,58,237,.1)',border:'1px solid rgba(124,58,237,.2)',color:'var(--accent3)',cursor:'default'}}>{b.emoji} {b.name}</div>))}</div></div>)}
              {overdue>0&&(<div style={{margin:'10px 2px 0',padding:'11px',background:'rgba(255,107,107,.07)',border:'1px solid rgba(255,107,107,.2)',borderRadius:12}}><p style={{fontSize:11,fontWeight:600,color:'var(--danger)',marginBottom:3}}>⚠ {overdue} Overdue</p><p style={{fontSize:11,color:'var(--muted)'}}>{overdue} task{overdue>1?'s':''} past deadline</p></div>)}
            </nav>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          <header style={{flexShrink:0,display:'flex',alignItems:'center',gap:11,padding:'12px 22px',background:'var(--surface)',borderBottom:'1px solid var(--border)'}}>
            ...
            <div style={{flex:1,minWidth:0}}><h1 style={{fontSize:15,fontWeight:700,color:'var(--text)',fontFamily:'Syne,sans-serif',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{headerTitle()}</h1><p className="header-date" style={{fontSize:11,color:'var(--muted)',marginTop:1}}>{new Date().toLocaleDateString('en-IN',{weekday:'long',day:'numeric',month:'long'})}</p></div>
            <NotificationBell />
            {view==='tasks'&&(<>
              <div className="header-search" style={{position:'relative'}}><span style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',color:'var(--muted)',fontSize:13,pointerEvents:'none'}}>⌕</span><input type="text" placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)} style={{paddingLeft:30,paddingRight:12,paddingTop:8,paddingBottom:8,borderRadius:10,background:'var(--surface2)',border:'1px solid var(--border)',color:'var(--text)',fontSize:13,outline:'none',width:170}} onFocus={focus} onBlur={blur}/></div>
              <div style={{display:'flex',gap:3,background:'var(--surface2)',borderRadius:9,padding:3,border:'1px solid var(--border)'}}>{[['list','☰'],['kanban','⊞'],['calendar','📅']].map(([mode,ic])=>(<button key={mode} onClick={()=>setViewMode(mode)} style={{width:30,height:26,borderRadius:6,border:'none',fontSize:13,cursor:'pointer',transition:'all .15s',background:viewMode===mode?'var(--accent)':'transparent',color:viewMode===mode?'#fff':'var(--muted)'}}>{ic}</button>))}</div>
          ...
          <div className="main-pad" style={{flex:1,overflowY:'auto',padding:22}}>
            ...
            :view==='pomodoro'?<PomodoroTimer tasks={tasks} onSessionComplete={()=>fetchAll()}/>
                ...
                {viewMode==='list'&&(<>
                  ...
                  {loading
                    ...
                    :filtered.length===0
                      ...
                      // ─── CHANGED: opens drawer on title, passes onBreakdown ──
                      :filtered.map((t,i)=>(
                          <TaskRow key={t.id} task={t} idx={i}
                            onToggle={()=>handleToggle(t.id)}
                            onEdit={()=>setDrawerTask(t)}
                            onDelete={()=>handleDelete(t.id)}
                            onBreakdown={()=>setBreakdownTask(t)}
                          />
                        ))
                  }
                </>)}
        ...
        <nav className="bnav" style={{justifyContent:'space-around',alignItems:'center'}}>
          {[['tasks','📋','Tasks'],['focus','🎯','Focus'],['pomodoro','⏱','Timer'],['leaderboard','🏆','Ranks'],['profile','👤','Me']].map(([v,ic,lb])=>(<button key={v} className="bnav-btn" onClick={()=>{setView(v);setSideOpen(false)}} style={{color:view===v?'var(--accent2)':'var(--muted)',fontWeight:view===v?700:400}}><span style={{fontSize:20,lineHeight:1,display:'block',transform:view===v?'translateY(-1px)':'none',transition:'transform .15s'}}>{ic}</span><span style={{fontSize:9,letterSpacing:'.3px'}}>{lb}</span>{view===v&&<div className="bnav-dot"/>}</button>))}
        </nav>

        {/* ─── NEW FEATURE 1: ⌘K Quick Add ──────────────────────────────── */}
        <GlobalQuickAdd token={token} apiUrl={import.meta.env.VITE_API_URL} onTaskAdded={fetchAll}/>

        {/* ─── NEW FEATURE 2: Task Detail Drawer ────────────────────────── */}
        <TaskDetailDrawer task={drawerTask} onClose={()=>setDrawerTask(null)} onUpdated={fetchAll} token={token} apiUrl={import.meta.env.VITE_API_URL}/>

        {/* ─── NEW FEATURE 3: AI Breakdown Modal ────────────────────────── */}
        <AIBreakdown task={breakdownTask} onClose={()=>setBreakdownTask(null)} onSubtasksAdded={fetchAll} token={token} apiUrl={import.meta.env.VITE_API_URL}/>

        {/* ─── NEW FEATURE 4+5: PWA Offline Banner ──────────────────────── */}
