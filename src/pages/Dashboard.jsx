...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes glow {
    ...
    50%     { box-shadow: 0 0 40px var(--glow), 0 0 80px rgba(124,58,237,0.15); }
  }
...
function StatCard({ label, value, sub, color, icon, delay = 0 }) {
  return (
    <div style={{
      ...
      <div style={{ position:'absolute', top:14, right:16, fontSize:28, opacity:.08 }}>{icon}</div>
      ...
      <p style={{ fontSize:34, fontWeight:800, color, lineHeight:1, marginBottom:6, fontFamily:'Syne, sans-serif' }}>{value}</p>
  ...
  return (
    ...
    >
      ...
      <button onClick={onToggle} style={{
        ...
        border:`2px solid ${isDone ? cfg.color : 'var(--border2)'}`,
      ...
      <span style={{
        ...
        border:`1px solid ${scfg.color}30`, flexShrink:0,
      }}>{scfg.emoji} {scfg.label}</span>
      ...
      <div style={{ display:'flex', gap:5, opacity:hover?1:0, transition:'opacity .15s', flexShrink:0 }}>
        ...
        <button onClick={handleDel} style={{
          ...
          background: confirmDel ? 'rgba(255,107,107,.15)' : 'var(--surface2)',
...
function TaskModal({ task, onClose, onSave }) {
  ...
  async function submit(e) {
    ...
    try {
      let dueDate = null
      if (f.dueDate) {
        const time = f.dueTime || '00:00'
        dueDate = `${f.dueDate}T${time}:00`
      }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        boxShadow:'0 30px 80px rgba(0,0,0,.7), 0 0 0 1px rgba(124,58,237,.1)',
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontSize:18, fontWeight:700, color:'var(--text)', fontFamily:'Syne, sans-serif' }}>
        ...
        <form onSubmit={submit} style={{ padding:22 }}>
          {err && (
            <div style={{ background:'rgba(255,107,107,.08)', border:'1px solid rgba(255,107,107,.2)', borderRadius:9, padding:'10px 14px', fontSize:12, color:'var(--danger)', marginBottom:14 }}>⚠ {err}</div>
          )}
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input autoFocus style={inp} type="text" placeholder="What needs to be done?"
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
            ...
            <div>
              ...
              <input style={inp} type="time" value={f.dueTime}
          ...
          <div style={{ display:'flex', gap:10, paddingTop:14, borderTop:'1px solid var(--border)', marginTop:4 }}>
            ...
            <button type="submit" disabled={loading} style={{
              ...
              boxShadow: loading ? 'none' : '0 4px 20px var(--glow)',
              transition:'all .2s',
            }}>
...
export default function Dashboard() {
  ...
  async function handleToggle(id) {
    ...
    try {
      ...
      fetchAll()
      flash('success', newStatus === 'DONE' ? '🎉 Task completed!' : '↩ Marked as to do')
          ...
          <aside style={{
            ...
            <nav style={{ flex:1, padding:'12px 10px', overflowY:'auto' }}>
              ...
              {[
                ...
                <button key={item.key} onClick={() => setFilter(item.key)} style={{
                  ...
                  background: filter===item.key ? 'rgba(124,58,237,.1)' : 'transparent',
                  color:      filter===item.key ? 'var(--accent2)'      : 'var(--muted)',
                  border:     filter===item.key ? '1px solid rgba(124,58,237,.2)' : '1px solid transparent',
              ...
              {[['📊','Analytics'],['🔥','Streaks'],['⚙️','Settings']].map(([ic,lb]) => (
                <button key={lb} style={{
                  ...
                  color:'var(--muted)', cursor:'pointer', fontSize:13, textAlign:'left',
              ...
              {tasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'DONE').length > 0 && (
                <div style={{
                  margin:'16px 2px 0', padding:'12px',
                  ...
                  <p style={{ fontSize:11, color:'var(--muted)' }}>
                    {tasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'DONE').length} task(s) past deadline
                  </p>
                </div>
              )}
            </nav>
            ...
            <div style={{ padding:'13px 14px', borderTop:'1px solid var(--border)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                ...
                <button onClick={() => { logout(); navigate('/login') }} title="Logout" style={{
          ...
          <div style={{ flex:1, overflowY:'auto', padding:24 }}>
            ...
            {loading ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'80px 0', animation:'fadeUp .4s ease' }}>
                ...
                <p style={{ fontSize:17, fontWeight:700, color:'var(--text)', marginBottom:6, fontFamily:'Syne, sans-serif' }}>
              ...
              <div>
                {filtered.map((task, i) => (
                  <TaskCard key={task.id} task={task} idx={i}
