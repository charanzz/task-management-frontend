...
function TaskRow({task,onToggle,onEdit,onDelete,idx}){
  ...
  return(
    <div className="task-row" onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',background:'var(--surface)',border:'1px solid var(--border)',borderRadius:14,marginBottom:8,transition:'all .18s',opacity:isDone?.6:1,animation:`slideIn .35s ease both`,animationDelay:`${idx*.04}s`,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
          <p style={{fontSize:14,fontWeight:500,color:isDone?'var(--muted)':'var(--text)',textDecoration:isDone?'line-through':'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',margin:0}}>{task.title}</p>
...
export default function Dashboard(){
  ...
  useEffect(()=>{
    if(pomRunning){pomRef.current=setInterval(()=>{setPomSecs(s=>{if(s<=1){clearInterval(pomRef.current);setPomRunning(false);if(pomMode==='focus')setPomSessions(p=>p+1);return 0}return s-1})},1000)}
  ...
  useEffect(()=>{api.get('/api/users/me').then(r=>{setRoleFromApi(r.data.role||'USER');if(r.data.createdAt){const diffDays=Math.floor((new Date()-new Date(r.data.createdAt))/(1000*60*60*24));setTrialDaysLeft(Math.max(0,15-diffDays))}if(user)login(localStorage.getItem('token'),{...user,role:r.data.role,isPro:r.data.isPro})}).catch(()=>{})},[])
      ...
      <div style={{display:'flex',height:'100vh',overflow:'hidden',background:theme==='light'?'#f4f4f9':'var(--bg)',color:theme==='light'?'#1a1a2e':'var(--text)','--bg':theme==='light'?'#f4f4f9':'#0a0a0f','--surface':theme==='light'?'#ffffff':'#111118','--surface2':theme==='light'?'#ededf4':'#1a1a24','--border':theme==='light'?'rgba(0,0,0,.09)':'rgba(255,255,255,.06)','--border2':theme==='light'?'rgba(0,0,0,.13)':'rgba(255,255,255,.1)','--text':theme==='light'?'#1a1a2e':'#f0f0f8','--muted':theme==='light'?'#555577':'#6b6b8a'}}>
              ...
              {[
                ...
                return(
                  <div key={group.key} style={{marginBottom:4}}>
                    <button onClick={()=>toggleNav(group.key)} style={{width:'100%',display:'flex',alignItems:'center',gap:8,padding:'8px 10px',borderRadius:10,cursor:'pointer',background:isGroupActive?'rgba(124,58,237,.06)':'transparent',border:`1px solid ${isGroupActive?'rgba(124,58,237,.12)':'transparent'}`,transition:'all .15s'}}>
                      <span style={{fontSize:14}}>{group.icon}</span>
                      <span style={{flex:1,fontSize:11,fontWeight:700,letterSpacing:'1px',textTransform:'uppercase',color:isGroupActive?'var(--accent2)':'var(--muted)',textAlign:'left'}}>{group.label}</span>
                      <span style={{fontSize:10,color:'var(--muted)',transition:'transform .2s',transform:isOpen?'rotate(90deg)':'rotate(0deg)'}}>›</span>
                    </button>
                    {isOpen&&(
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div className="main-pad" style={{flex:1,overflowY:'auto',padding:22}}>
            ...
            :view==='calendar'?<CalendarView tasks={tasks} onTaskClick={t=>{setEditTask(t);setModal(true)}} onDayClick={()=>{setModal(true)}}/>
